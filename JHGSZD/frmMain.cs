...
namespace JHGSZD
{
    public partial class frmMain : Form
    {
        ...
        private static void genDataForTest()
        {
            for (int j = 0; j < 4; j++)
            {
                for (int i = 1; i < clsPublicStatic.intRainNum[j]; i++)
                {
                    clsPublicStatic.rainPositionHour[j, i] = DateTime.Now.Second + i;
                }

                for (int i = 1; i < clsPublicStatic.intWindNum[j]; i++)
                {
                    clsPublicStatic.windPositionV[j, i] = Math.Abs(35 - DateTime.Now.Second + i / 5) + 0.2;
                }
            }
        }
